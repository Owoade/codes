...
burger.addEventListener('click', () => {
    ...
    dropdownLinks.forEach((links, index) => {
        if (links.style.animation) {
            ...
            links.style.animation = `slideIn .4s forwards ${index*0.4 + 0.2}s`
        }
...
function scrollEffect() {
    works.forEach((work, index) => {
        if (work.getBoundingClientRect().top < window.innerHeight) {
            work.style.animation = `slideIn .4s forwards ${index/9}s`;
        }
    })
}
...
let words = ['beautiful websites', 'awesome graphic designs', 'top notch UI designs'],
